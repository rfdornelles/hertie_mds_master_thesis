...
HERTIE_OPENAI_ORG_ID = os.getenv("HERTIE_OPENAI_ORG_ID")
HERTIE_OPENAI_PROJECT_ID = os.getenv("HERTIE_OPENAI_PROJECT_ID")

#### definitions
model = 'gpt-4.1-2025-04-14' #'ft:gpt-4o-mini-2024-07-18:hertie-school:judicial-sentences:BKQ48gf0'
...
if model.startswith("o"):
  print(f"[ATTENTION] Using the OpenAI model {model} - this model doesnt' support max tokens")
...
client = OpenAI(
  api_key=API_KEY,
  organization=HERTIE_OPENAI_ORG_ID,
  project=HERTIE_OPENAI_PROJECT_ID,
  )
...
if os.path.exists(f"{experiment}-batch.jsonl"):
  ...
  # if the model name os `o_xxx' the parameter is called max_completion_tokens
  max_tokens_name = "max_completion_tokens" if model.startswith("o") else "max_tokens"
                                                            
  max_tokens_value = 3000 if model.startswith("o") else 1000
  ...
  for index, row in df.iterrows():
      ...
      body = {
          "model": model,
          #max_tokens_name: max_tokens_value,
          "messages": [
      ...
      if not model.startswith("o"):
          body["temperature"] = 0
          body[max_tokens_name] = max_tokens_value
...
def get_batch_job_status(batch_job_id):
    ...
    if(batch_job_status.status == "validating"):
      print("Batch job is validating...")
      return batch_job_status, 0.1
    
    if (batch_job_status.status == "finalizing"):
        print(f"Batch job is finalizing: {batch_job_status.status}")
        return batch_job_status, 0.01
          
    elif (batch_job_status.status == "processing"):
...
status, wait_time = get_batch_job_status(batch_job_id) 
...
while status.status not in ["completed", "failed"]:
    ...
    status, wait_time = get_batch_job_status(batch_job_id)  # update status and wait_time
    ...
    if total > 0:
        print(f"Total requests: {total} - Completed: {completed} ({completed/total*100:.2f}%) - Failed: {failed} ({failed/total*100:.2f}%)")
    else:
        print(f"Total requests: {total} - Completed: {completed} - Failed: {failed}")

 
    
    time.sleep(60*wait_time) # wait for 5 minutes
    status, wait_time = get_batch_job_status(batch_job_id)
...
if os.path.exists(output_path):
    ...
    print(f"Batch job metadata saved to {output_path}")
