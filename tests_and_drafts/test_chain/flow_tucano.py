...
def generate_answer(prompt, max_new_tokens=526):
    """
...
def process_document(doc_text, fields=field_prompts, top_k=3):
    ...
    print(f"[DEBUG] Para doc {doc_id}, geramos {len(chunks)} chunks.")
...
df = pd.read_parquet("../master_thesis/validation.parquet")[0:5]
...
def process_document_grouped(doc_text, grouped_fields_dict, top_k=3):
    ...
    for group_name, fields_in_group in grouped_fields_dict.items():
        ...
        retrieved = retrieve_chunks(query_prompt, index, chunks, top_k=top_k)
        print(f"[DEBUG] Grupo: {group_name} - retrieved chunks:\n", retrieved)
        ...
        try:
            ...
            print("[DEBUG] Falha ao fazer parse do JSON, resposta era:\n", llm_response)
...
results_df.to_csv("/home/228446@hertie-school.lan/workspace/test_chain/resultados_extracao_validacao_grouped_tucano.csv", index=False)
