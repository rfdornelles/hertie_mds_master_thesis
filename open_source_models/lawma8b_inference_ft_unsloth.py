...
# definitions
experiment = 'experiment_lawma_8b_unsloth_inference'
folder = f'../experiments/{experiment}'

os.makedirs(folder, exist_ok=True)

## Configuration
ckpt = "rfdornelles/master_thesis_lawma8b_ft" #"unsloth/Phi-4"
max_seq_length = 2048 * 9
num_train_epochs = 10
full_finetuning = False
now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
experiment_name = f"run_unsloth_ft_{'full' if full_finetuning else 'lora'}_{ckpt.split('/')[1]}_v1"
...
def run_model(processo, new_tokens=1000, overwrite = False):
  ...
  # build the prompt
  message = [
            {'role': 'user', 'content': prompt + f"----- Sentença judicial:     {sentenca}"},
        ]
