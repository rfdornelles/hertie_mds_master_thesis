...
<!--TODO: REMOVE FT MODEL ---> 
![Tasks by accuracy](../../evaluate_results/task_accuracy_heatmap.png){#fig-base_model}

In nearly all models, the open textual tasks performed very well, while the performance of boolean and categorical tasks varied more significantly. Some differences were noted in numeric tasks, although they all performed well overall. Also, we can breakdown better by NLP task @sec-nlp-tasks

<!--TODO: explain the columns, their task, show the number of tasks in each  --->

## Fine-tuned results

Those results suggested that continuing to work with open-source models was a promising strategy. So, fine-tuning was performed with the models OpenAI 4o-mini (which is the recommended model by OpenAI), Llama 3.2 (3B), and Phi4. No resources were available to perform fine-tuning on other models. Despite many attempts, some of them, mainly from the Gemma family, lacked full library support. 

![Evolution in loss during fine-tuning](../../evaluate_results/finetune_losses.png){#fig-ft_loss_evolution}




After the fine-tuning, we could see a big improvement in the results.
