...
From this comparison, we will have a matrix with all answers and a value of 0 if they do not match or 1 if they are right. Then, the sum of correct answers will be divided by the total answers expected to assess the accuracy. The accuracy was calculated using scikit-learn's function `metrics.accuracy_score` @buitinck_api_2013, which uses the following formula ^[https://scikit-learn.org/stable/modules/model_evaluation.html#accuracy-score]:
...
Therefore, each model would achieve a value between 0 and 1 according to its capabilities to generate correct answers. This strategy also evaluates the accuracy in specific columns, groups of columns, and other combinations, as needed. 
