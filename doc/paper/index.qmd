...
![Illustration of the extraction of information from a judicial decision. There are multiple tasks that depend on understanding the text and following rules](images/schema-extraction.png){#fig-schema-extraction width="500px"}
...
Within this framework, choosing the more suitable LLMs for the task would be possible. As discussed further, 12 models were tested with the test dataset (containing 39 judicial sentences). 
...
The choice to use accuracy as the ideal metric is because the goal is to understand how close the answers are to the ground truth, as used in other papers such as (@dominguez-olmedo_lawma_2024; @junior_juru_2024). Although some sub-tasks exist—such as classifying the decision's outcome or comparing the numeric answers—we would need diverse metrics like f1 or mean squared error, then uniformly transformed into a single value to assess how each model has performed. This would heavily sacrifice interpretability without clear gains in understanding the performance.
...
According with @eq-accuracy, each model would achieve a value between 0 and 1 according to its capabilities to generate correct answers. This strategy also allows us to evaluate the accuracy in specific columns, groups of columns, and other combinations, as needed. 
...
To overcome the computational power limitation, we relied on Google Colab (both standard and pro versions) and the Hertie School GPU Server^[https://hertie-data-science.gitbook.io/hertie-school-gpu/extras/gpu-server-specs] ^[Despite some throttling issues caused by the high number of simultaneous users and the absence of a job management system, this research would not have been possible without the use of the GPU server. A deep thank you to the Data Science Lab for their support!].
...
<!--TODO: REFS ---> 
Some techniques, such as Yarn ***REFERENCE*** and Rope scaling ***REFERENCE***, were attempted to increase those models' context window artificially, but they would demand a heavy processing time, which does not fit under the scope of this research. 
...
The solution was to discard models such as LoLA, TucanoBR, and Lawma in favor of other general-use models with a larger context window. Regarding the output format, it was forced into a uniform JSON, using Langchain (@chase_langchain_2022) and Pydantic (@colvin_pydantic_2025) as mentioned above.
...
Each model received batches containing the prompt and each of the documents in the test dataset [@sec-datasets ]. The results were saved into individual `.json` files in the respective folder and evaluated according to the parsing and evaluation strategies [@REF models].

<!--- TODO: Check if all models were run using HF ---> 

## Pipeline to fine-tune models
@junior_juru_2024 ; 


### OpenAI's models
In the case of OpenAI's models, the training pipeline was generated using the API and run on their own servers [https://platform.openai.com/docs/guides/fine-tuning]. To meet its requirements, a pipeline was built to upload each document in batches to the training dataset, specifying the model and the general parameters.  
...
Regarding the open-source models it was needed to run it locally -- as it was necessary with the inference. However, 
...
-- @raschka_build_2024, Lora - 'A rank and alpha of 16 are good default choices'
...
[^2]: There is the [mini version of Phi-4](https://huggingface.co/microsoft/Phi-4-mini-instruct), however it was not possible to run or fine-tune it with the actual computational resources and due a lack of support from the Unsloth framework. Ref: https://github.com/unslothai/unsloth/issues/2359
