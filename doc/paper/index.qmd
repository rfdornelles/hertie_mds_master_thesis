...
Specific instructions were provided regarding the output. Since the date could not vary in its format—otherwise, it would not be possible to transform it into a structured data frame—the model received specific instructions to provide the answer in a valid JSON format, following a given data schema.

## 2. Pipeline to run commercial models
...
So, to run each desired model, a batch was prepared, and the model and the parameters were chosen. The file was uploaded, a batch generation was started, and the results were downloaded back when they were ready. Regarding the parameters, all OpenAI models were run with temperature at 0 level, specifying the max tokens to be generated in 1.000, except with the reasoning models without this option.
...
For instance, by taking the example below in [@REF TABLE], we can see the correct values in the desired JSON format. 
[TABLE 1]

The following examples include several cases of outputs that are materially correct but in the wrong format. As we can see, we would have problems parsing them in a structured data frame.
...
From this comparison, we will have a matrix with all answers and a value of 0 if they do not match or 1 if they are right. Then, the sum of correct answers will be divided by the total answers expected to assess the accuracy. 

[[formula]] 

Therefore, each model would achieve a value between 0 and 1 according to itsto its capabilities to generate correct answers. This strategy also allows evaluating the accuracy in specific columns, groups of columns, and other combinations, as needed. 

## 5. Pipeline to run open-source models

--- challenging - lack of computational resources


## 6. Pipeline to fine-tune models
...
tibble::tibble(
  ...
  `Why is this invalid?` = c(
    ...
    "List without field identifiers; lacks explicit mapping to `processo`, `juiz`, `cocaina_g`, `sentenca`, and `tot_pen`.",
    ...
    "Custom syntax not aligned with standard field names (`processo`, `juiz`, `cocaina_g`, etc.); difficult to parse programmatically."
  ),
) |> 
  dplyr::mutate(dplyr::across(.fns = gt::escape_latex)) |> 
  ...
  kableExtra::kable_styling(
    ...
    full_width = FALSE,
    font_size = 9) |> 
  #kableExtra::add_header_above(c(" " = 2)) |>  # Optional visual break
...
#| tbl-cap: Example of correct output.
