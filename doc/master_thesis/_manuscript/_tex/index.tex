...
Summary\footnote{The content of this research is available at:
  \url{https://github.com/rfdornelles/hertie_mds_master_thesis}}}{Executive Summary}}\label{sec-executive-summary}

\textbf{Executive Summary}

Brazil's courts are almost entirely digital, yet legal scholarship still
advances case-by-case, slowed by manual reading and tiny samples. This
thesis shows that \emph{open-source} large-language models (LLMs) can
break that bottleneck. With a carefully engineered prompt and a few
hundred labelled examples, an open model can extract fifty legal
features from full-text judgments with \textbf{over 90 \%
accuracy}---performance once thought to require expensive proprietary
APIs.

With modest compute and a few hundred labels, open-source LLMs make
large-scale, evidence-based legal
research---\textbf{jurimetrics}---practical for Brazil's academic,
public-sector, and civil-society communities.

\textbf{Why it matters}\\
Millions of judicial decisions already sit online, but turning them into
structured data is labour-intensive. Universities, watchdog NGOs, and
public agencies need low-cost, privacy-preserving tools; commercial LLM
services remain pricey and opaque.

\textbf{What we did}\\
- Adapt a \textbf{gold-standard corpus} of 258 São Paulo
drug-trafficking sentences, each originally hand-labelled with 50
attributes.\\
- Benchmarked \textbf{twelve leading models} (GPT-4 variants, Gemma 3,
Phi-4, Llama 3, etc.) on extraction accuracy.\\
- Applied \textbf{few-shot fine-tuning} (≈200 labelled cases, 4-bit LoRA
via the Unsloth framework) to the most promising open models.

\textbf{Key findings}\\
Fine-tuning boosts open-source accuracy by \textbf{+20 percentage
points}: Phi-4 climbs to 0.92, effectively matching GPT-4o-mini while
cutting inference cost by 25 \%. A 3 billion-parameter Llama reaches
0.75 accuracy, runs twice as fast, and costs half as much as
GPT-4o-mini---ideal for rapid screening at scale.

\textbf{Contributions}\\
1. A \textbf{reproducible R + Python pipeline} for scraping, prompting,
parsing, and scoring legal texts.\\
2. A \textbf{demonstration dataset} of 454 fresh São Paulo judgments
processed by fine-tuned models, proving scalability.\\
3. A \textbf{policy roadmap} urging courts to publish structured data,
law schools to teach data literacy, and funders to support open corpora.
...
\begin{quote}
\emph{``Give me the data, and I will give you the law.''}\\
--- \emph{adapted from the scholastic motto} \textbf{da mihi data, dabo
tibi ius}
\end{quote}

Law shapes public policy, fuels political debate, and touches the daily
lives of Brazilians.\\
Yet empirical legal research remains surprisingly scarce.\\
Although recent work by Cabrera de Bonito
(\citeproc{ref-cabrera_de_bonito_pesquisa_2025}{Cabrera De Bonito et
al., 2025}) maps a growing interest in data-driven methods, most studies
still rely on close reading, modest samples, and painstaking human
coding.

Paradoxically, the raw material is abundant. Since 2010 Brazilian courts
have been fully digital: every petition, ruling, and vote leaves a
machine-readable trail. Exploratory projects that mine only metadata
have already yielded insight. Dornelles
(\citeproc{ref-dornelles_o_2021}{Dornelles, 2021}), for example, charted
the surge of new actions filed at the Supreme Court across presidential
terms ((\citeproc{ref-ref}{\textbf{ref?}})(fig-evolution)). But metadata
sketches only the shoreline. To sail further---asking \emph{why} cases
swing one way or another, or probing subtle patterns of
bias---researchers must dive into the dense, jargon-laden text of
judicial opinions. Doing so at scale traditionally requires either a
small army of lawyers or brittle NLP pipelines that break whenever the
template shifts.

Large-language models (LLMs) promise a way out. Unlike traditional NLP
tools that master a single trick, modern LLMs handle long contexts,
follow intricate instructions, and juggle multiple tasks
(\citeproc{ref-raschka_build_2024}{Raschka, 2024};
\citeproc{ref-vajjala_practical_2020}{Vajjala et al., 2020}). Crucially,
their power is no longer locked behind proprietary APIs: a new wave of
\emph{open-source} models---\textbf{Phi-4}, \textbf{Gemma 3}, and a
family of nimble \textbf{Llamas}---offers comparable capability with
full local control over data, cost, and fine-tuning.

To test this, I assemble a gold-standard corpus of 258 São Paulo
drug-trafficking sentences annotated by legal experts
(\citeproc{ref-lacerda_e_silva_punindo_2021}{Lacerda e Silva, 2021}),
benchmark twelve leading models, fine-tune the most promising ones on a
few hundred labelled examples, and evaluate performance across numeric,
categorical, boolean, and free-text tasks.
...
\tightlist
\item
  \textbf{Reproducible pipeline} -- an R + Python workflow that scrapes,
  structures, and scores legal texts, ready for other courts or
  languages.\\
\item
  \textbf{Empirical evidence} -- showing that fine-tuning on ≈200 cases
  can push an open-source model beyond 90 \% accuracy, closing the gap
  with GPT 4o-mini while cutting inference costs by \textasciitilde25
  \%.\\
\item
  \textbf{Public assets} -- a released dataset and codebase to lower the
  barrier for future data-driven legal studies.
\end{enumerate}

In short, this thesis aims to move Brazilian legal scholarship from
isolated close readings to scalable, evidence-rich analysis---bringing
the promise of jurimetrics a decisive step closer to reality.

\section{Related Work}\label{related-work}

The works most closely related to this thesis are those of
Dominguez-Olmedo et al.
(\citeproc{ref-dominguez-olmedo_lawma_2024}{2024}) and Junior et al.
(\citeproc{ref-junior_juru_2024}{2024}), who concluded that fine-tuning
models for a specific legal context lead to notable improvements in
accuracy. From a technical perspective, this research heavily relied on
Wolf et al. (\citeproc{ref-wolf_transformers_2020}{2020}) and the Daniel
Han \& team (\citeproc{ref-daniel_han_unsloth_2023}{2023}) libraries,
which provided the necessary tools for fine-tuning open-source models.

\subsection{Teorethical Framework}\label{sec-teorethical-framework}

This research relies on data science Wickham et al.
(\citeproc{ref-wickham_r_2023}{2023}) applied to natural language
processing tasks Vajjala et al.
(\citeproc{ref-vajjala_practical_2020}{2020}), specifically large
language models (Raschka (\citeproc{ref-raschka_build_2024}{2024}))
powered by recent advancements in the field due to deep learning
techniques Zhang et al. (\citeproc{ref-zhang_dive_2023}{2023}).
...
law to `its concrete manifestation'\,' (Alves da Silva
(\citeproc{ref-alves_da_silva_aspectos_2022}{2022})).
...
contributed to criminal law, describing how judicial decisions evaluate
the line between a drug user and a drug dealer according to the amount
of drug seized.
...
(\citeproc{ref-loevinger_jurimetricsnext_1948}{1948})) is also,
according to Cabrera De Bonito et al.
...
largely due to methodological gaps from practitioners (Andrade, 2018,
quoted by Cabrera De Bonito et al.
...
the remaining cases'' Dominguez-Olmedo et al.
...
With the digitalization of the Brazilian Judiciary branch, extracting
this data from the source is not a significant technical challenge.
However, obtaining labels and annotations to complement the raw text of
the judicial decisions is a precondition for applying this method.
...
It was possible to reach out to the researcher in charge of a very
carefully annotated dataset, Lacerda e Silva
(\citeproc{ref-lacerda_e_silva_punindo_2021}{2021}), which was used in
her master's thesis. In this research, despite focusing only on São
Paulo's Court of Justice (which is, by its turn, the biggest Court in
Latin America), annotations were made in about 400 legal cases related
to drug trafficking. Access to the dataset was obtained by contacting
the researcher, who provided it for further adaptation to this research.

The researcher collected these cases from the São Paulo State Court of
Justice (TJSP), focusing on a sample covering 2017 and 2018.
Approximately 35 features were manually annotated and labeled for each
case. Some of the original features also referred to other documents
rather than the judicial decision, and, therefore, they were excluded
(since it would not be reasonable that the model inferred that).
Features related to biological data were also excluded for ethical
reasons.
...
al., 2024}), in Portuguese Pires et al.
(\citeproc{ref-pires_sabia_2023}{2023}), or representative of the
state-of-the-art Team et al. (\citeproc{ref-team_gemma_2025}{2025}). Of
course, there were very high-quality models such as DeepSeek, Mistral,
and many others, but it was not possible to experiment with them all. In
...
more), but it is also possible that they are way bigger than that.
...
Generation (RAG) (\citeproc{ref-lewis_retrieval-augmented_2021}{Lewis et
al., 2021}) were also attempted, with no good results. Besides their
complexity, the model would be unable to understand the document as a
whole. Although theoretically, it would be capable of extracting
entities, it would fail in tasks such as evaluating whether there are
inconsistencies in the police officer's account since it requires a
...
information, using transfer
learning(\citeproc{ref-vajjala_practical_2020}{Vajjala et al.,
2020})\footnote{``Recently, large transformers have been used for
...
using the API and run on their servers\footnote{https://platform.openai.com/docs/guides/fine-tuning}.
...
applying LoRA to train only the small amount of layers possible, usually
less than 1\%. Following suggestions, we applied a rank and alpha of 16
Raschka (\citeproc{ref-raschka_build_2024}{2024}), which led to a tiny
...
To answer the research question Section~\ref{sec-research_question}, we
need to understand how commercial and open-source models perform in a
...
In nearly all models, the open textual tasks performed very well, while
the performance of boolean and categorical tasks varied more
significantly. Some differences were noted in numeric tasks, although
they all performed well overall.
...
\includegraphics[width=6.25in,height=\textheight,keepaspectratio]{../../evaluate_results/finetune_losses.png}
...
As Figure~\ref{fig-finetune} showed, model Phi4 tied with GPT 4o-mini
and the much smaller Llama 3.2 (3B) learned very easily, even with both
models being trained with the same 10 epochs.\footnote{With a richer
  dataset and/or more epochs, it would be possible to reach or even
  surpass GPT 4o-mini performance.}
...
minutes for the entire batch (equating to about 16 seconds per sample),
while Phi-4 took 4 hours, 7 minutes to process the same batch, resulting
in significantly longer individual sample times. When considering costs,
we can compare the three models by examining the energy expenditures of
the open-source models\footnote{Estimated energy costs: €0.2889 per KWh,
  EU average price in the first half of 2024, according to
  https://ec.europa.eu/eurostat/statistics-explained/index.php?title=Electricity\_price\_statistics\#Electricity\_prices\_for\_household\_consumers}
...
fine-tuned models\footnote{Assuming batch prices, which are the most
  ...
  https://platform.openai.com/docs/pricing. Assuming 1 USD = 0.88 EUR}.
...
Also, we cannot affirm that the same.
...
We have found Figure~\ref{fig-base_model} that commercial and
open-source models can potentially enable jurimetric research. Many of
them achieve \textgreater{} 0.7 points of accuracy in a zero-shot
setting, and GPT 4o-mini and Phi-4 achieve \textgreater{} 0.92 after
fine-tuning. This learning happens in all kinds of tasks
(Table~\ref{tbl-finetune-delta}).

Even with a small dataset of 186 cases, applying LoRA and Quantization
through (\citeproc{ref-unsloth}{\textbf{unsloth?}}) enabled the
open-source models to learn the patterns quite well, as
Figure~\ref{fig-ft_loss_evolution} shows an important decrease in the
validation loss. Results suggest that even fewer epochs could have been
used.
...
deployments. It would be ideal to have a few thousand judicial sentences
...
Efficiency is another important future path of work. Trying techniques
to prune models and distill their content would lead to lighter and more
efficient models. Once models such as Tucano or Sabiá have larger
windows of context and are optimized for instruction tuning, they might
be tested as well.
...
Lastly, although the results are specific to the Brazilian context,
similar approaches might be attempted in other jurisdictions and even
with different languages as long as the base models have idiomatic
support and there is enough training data in the legal task.
...
relevant to replicating (\citeproc{ref-lacerda}{\textbf{lacerda?}})'s
work in recent years, especially after the Brazilian Supreme Court
decided on drug decriminalization\footnote{https://www.conectas.org/en/noticias/nine-years-into-the-case-the-brazilian-federal-supreme-court-stf-decriminalizes-possession-of-marijuana-for-personal-use/}.
...
\subsection{Human in the loop}\label{human-in-the-loop}

However, it would be strongly advisable not to shorten or cut research
job positions in favor of ``substituting'' human labor with a large
language model.

Domain knowledge was a key factor in success throughout the process. To
believe that it could be simply `automated' would be a big mistake since
the conception and design need a deep understanding of the legal field
under analysis. Not having enough human oversight during operation would
lead to mistakes being massively replicated and more subtle biases not
being identified.
...
possible, as it would be a valuable resource for further fine-tuning
...
Judiciary branch: The judiciary branch should ensure data and lawsuit
documents are available digitally for research while maintaining high
privacy standards. Even if anonymized, keeping the data open in robust
systems such as DataJud\footnote{https://www.cnj.jus.br/sistemas/datajud/}
and the TJ-SP Judicial Decisions Database\footnote{https://esaj.tjsp.jus.br/cjpg/}
would ease access to legal documents and allow further research.

Law schools: Adapt the Law curriculum so scholars are data literate.
Future professionals should be aware of technology but especially
empowered by the new tools that artificial intelligence and data science
can already provide them.

Think tanks and research networks: Invest in jurimetric research,
gathering legal professionals and data scientists to strengthen
methodology and empirical evidence as much as possible.

Open-source community: Keep offering high-quality frameworks such as
Torch, HuggingFace, and Unsloth. The deployment of new LLMs would be
ideal if they maintained larger context windows (at least 20k tokens)
...
\section{Appendix - Prompt}\label{sec-annex-prompt}

Lorem ipsum

\section{Appendix - Fine tune performance}\label{sec-ft-performance}
...
\begin{longtable}[t]{>{\raggedright\arraybackslash}p{15em}>{\raggedright\arraybackslash}p{17em}>{\raggedright\arraybackslash}p{17em}>{\raggedright\arraybackslash}p{20em}}

\toprule
column & data\_type\_task & nlp\_task & explanation\\
\midrule
\cellcolor{gray!10}{maconha\_g} & \cellcolor{gray!10}{numeric} & \cellcolor{gray!10}{numeric\_qa} & \cellcolor{gray!10}{extract numeric value}\\
cocaina\_g & numeric & numeric\_qa & extract numeric value\\
\cellcolor{gray!10}{crack\_g} & \cellcolor{gray!10}{numeric} & \cellcolor{gray!10}{numeric\_qa} & \cellcolor{gray!10}{extract numeric value}\\
ecstasy\_g & numeric & numeric\_qa & extract numeric value\\
\cellcolor{gray!10}{lsd\_g} & \cellcolor{gray!10}{numeric} & \cellcolor{gray!10}{numeric\_qa} & \cellcolor{gray!10}{extract numeric value}\\
\addlinespace
tot\_pen\_meses & numeric & numeric\_qa & extract numeric value\\
\cellcolor{gray!10}{aval\_antecedentes} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
aval\_conduta & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{aval\_personalidade} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
aval\_natureza & boolean & binary\_classification & binary classification flag\\
\addlinespace
\cellcolor{gray!10}{aval\_quantidade} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
aval\_variedade & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{aval\_circunstancias} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
aval\_consequencias & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{aval\_culpabilidade} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
\addlinespace
resultado\_art\_28 & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{resultado\_art\_33} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
resultado\_art\_34 & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{resultado\_art\_35} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
denuncia\_art\_33 & boolean & binary\_classification & binary classification flag\\
\addlinespace
\cellcolor{gray!10}{denuncia\_art\_34} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{binary\_classification} & \cellcolor{gray!10}{binary classification flag}\\
denuncia\_art\_35 & boolean & binary\_classification & binary classification flag\\
\cellcolor{gray!10}{maconha} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
maconha\_outras & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{cocaina} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
\addlinespace
crack & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{ecstasy} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
lsd & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{outras\_drogas} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
flag\_local\_de\_trafico & boolean & yesno\_qa & yes/no QA flag\\
\addlinespace
\cellcolor{gray!10}{flag\_preso\_no\_momento\_da\_sentenca} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
flag\_confissao\_informal & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{flag\_confissao} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
flag\_denuncia\_anonima & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{flag\_denuncia} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
\addlinespace
flag\_atitude\_suspeita & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{flag\_divergencias\_nos\_relatos\_dos\_policiais} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
flag\_investigacao & boolean & yesno\_qa & yes/no QA flag\\
\cellcolor{gray!10}{flag\_interceptacao} & \cellcolor{gray!10}{boolean} & \cellcolor{gray!10}{yesno\_qa} & \cellcolor{gray!10}{yes/no QA flag}\\
flag\_mandado & boolean & yesno\_qa & yes/no QA flag\\
\addlinespace
\cellcolor{gray!10}{local} & \cellcolor{gray!10}{categorical} & \cellcolor{gray!10}{multiclass\_classification} & \cellcolor{gray!10}{multi-class classification}\\
sentenca & categorical & multiclass\_classification & multi-class classification\\
\cellcolor{gray!10}{juiz} & \cellcolor{gray!10}{open\_textual} & \cellcolor{gray!10}{open\_text\_normalization} & \cellcolor{gray!10}{normalize text field}\\
nome & open\_textual & open\_text\_normalization & normalize text field\\
\cellcolor{gray!10}{pena\_base} & \cellcolor{gray!10}{span} & \cellcolor{gray!10}{named\_entity\_recognition} & \cellcolor{gray!10}{extract text span}\\
\addlinespace
tot\_pen & span & named\_entity\_recognition & extract text span\\
\bottomrule
